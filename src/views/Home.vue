 ...
 <div class="home">
    <div class="head d_flex">
      ...
      <div class="icons d_flex">
        <svg @click="getContacts" class="iconSvg" id="updateContacts" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M13.645 2.35C12.195 0.9 10.205 0 7.995 0C3.575 0 0.00500488 3.58 0.00500488 8C0.00500488 12.42 3.575 16 7.995 16C11.725 16 14.835 13.45 15.725 10H13.645C12.825 12.33 10.605 14 7.995 14C4.685 14 1.995 11.31 1.995 8C1.995 4.69 4.685 2 7.995 2C9.655 2 11.135 2.69 12.215 3.78L8.995 7H15.995V0L13.645 2.35Z" fill="black" fill-opacity="0.54"/>
        </svg>
        <svg @click="output" class="iconSvg" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M2 0H16C17.1 0 18 0.9 18 2V16C18 17.1 17.1 18 16 18H2C0.89 18 0 17.1 0 16V12H2V16H16V2H2V6H0V2C0 0.9 0.89 0 2 0ZM8.5 14L7.09 12.59L9.67 10H0V8H9.67L7.09 5.41L8.5 4L13.5 9L8.5 14Z" fill="black" fill-opacity="0.54"/>
        </svg>
      </div>
    </div>
    <div class="frame d_flex">
      <div class="category frame-border">
        <div class="category__row" v-for="storage in contactStorages" :key="storage.id" @click="getContactsInfo(storage.Id)">
          ...
          .then(res => {
                if (localStorage.getItem(storage) !== String(res.data.Result.CTag) || !storageArr) {
                  ...
                  for (let i = 0; i < contactsInfo.length; i++) {
                    this.uuids.push(`"${contactsInfo[i].UUID}"`)
                  }
                  this.GetContactsByUids(token)
                }
              }
          )
    },
    GetContactsByUids(token) {
      let bodyFormData = new FormData();
      bodyFormData.append('Module', 'Contacts');
      bodyFormData.append('Method', 'GetContactsByUids');
      bodyFormData.append(
          'Parameters', `{"Storage":"${this.currentStorage}","Uids":[${this.uuids}]}`
      )
...
.category__row-h2 {
  ...
  font-size: 16px;
  line-height: 24px;
  letter-spacing: 0.15px; color: rgba(0, 0, 0, 0.87);
